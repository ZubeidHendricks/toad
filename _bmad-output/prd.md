...
| D5 | Runtime substrate | **`@anthropic-ai/sdk` directly**, wrapped by a thin `@toa/runtime`. Prompt caching on; latest Claude models; per the `claude-api` skill. **Not** the Agent SDK for MVP. | Minimal deps, full control of the tool loop, Anthropic-native. |
...
- **FR9** Emitted code imports tools from the co-located `.tools.ts` and the
  agent loop from `@toa/runtime`; no logic is inlined that belongs in the runtime
  (service-layer rule).
...
- **E0 — Repo scaffold.** pnpm + Turbo monorepo; `@toa/compiler`, `@toa/runtime`;
...
- Whether `@toa/runtime` and the `toac` bin live in one package or two.
