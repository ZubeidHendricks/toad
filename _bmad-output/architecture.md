...
```
 ...
 ⑥ codegen → readable <name>.ts                              (Toa)
   ...
   ▼
 toad-runtime  ──► @anthropic-ai/sdk  (tool loop, structured output, caching)
```
...
## 3. Compiler pipeline (`toad-compiler`)
...
## 4. Data structures (`toad-compiler`)
...
toa/
  ...
  packages/
    runtime/    toad-runtime    defineTool, createAgent, run loop, errors
    compiler/   toad-compiler   pipeline + `toac` bin
  examples/researcher/          researcher.agent + .tools.ts (+ generated .ts)
  pnpm-workspace.yaml · turbo.json · tsconfig.base.json
```

```
emitted .ts ─► toad-runtime ─► @anthropic-ai/sdk, zod
toad-compiler ─► @toon-format/toon, zod      (build-time; NOT runtime)
toad-compiler ─X─► toad-runtime              (no dependency — one-way)
```

`toac` bin lives in `toad-compiler` (single package for MVP; split later if a
...
| Runtime vs bin packaging | `toac` in `toad-compiler`; `toad-runtime` separate (§7). |
